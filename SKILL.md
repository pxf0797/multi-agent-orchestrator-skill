...
|------|---------|--------------------------------|
| `code_dev` | 软件开发 SOP | 1.需求分析(Architect) → 2.并行开发(Developer×N) → 3.集成测试(QA) → 4.代码审查(Reviewer) |
| `deep_research` | 研究报告 SOP | 1.课题拆解(Researcher) → 2.并行搜索(Researcher×N) → 3.分类整理(Writer×N) → 4.报告合成(Writer) |
...
```
任务类型 → 角色匹配：
  编码/实现 → Developer（角色模板: role+goal+backstory+skills+constraints）
  设计/架构 → Architect
  测试/验证 → QA
  搜索/收集 → Researcher
  写作/整理 → Writer
