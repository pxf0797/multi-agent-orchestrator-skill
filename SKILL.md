...
|------|---------|--------------------------------|
| `code_dev` | 软件开发 SOP | 1.需求分析(Coordinator) → 2.并行开发(Developer×N) → 3.质量验证(Verifier) → 4.集成测试(QA) → 5.代码审查(Reviewer) |
  ...
  "tasks": [
    {
      ...
      "model": "haiku|sonnet|opus|DeepSeek-v4-flash|DeepSeek-v4-pro",
      "description": "详细任务描述",
      "depends_on": [],
...
调度循环前置步骤:
  ...
  → 启动事件消费者（调度循环内轮询读取 seq_tracker 游标后的新行）
  → 非 Monitor 持续监听：通过调度循环每周期读取新事件（避免通知风暴）
...
**角色模板加载：**
调度前，根据任务类型从角色模板库匹配角色定义，注入 Agent prompt：
```
任务类型 → 角色匹配：
...
Agent(
  ...
  model: "<根据任务类型选择：搜索用 haiku/DeepSeek-v4-flash，开发用 sonnet/DeepSeek-v4-flash，汇总用 opus/DeepSeek-v4-pro>",
...
| 架构设计 Agent | 大模型（Opus/DeepSeek-v4-pro）— 需要全局设计视角 |
| 审查 Agent | 大模型（Opus/DeepSeek-v4-pro）— 需要深度分析能力 |
| 测试/QA Agent | 中模型（Sonnet/DeepSeek-v4-flash）— 常规测试验证 |
| 验证 Agent | 中模型（Sonnet/DeepSeek-v4-flash）— 质检评判；Strict 模式升级为大模型 |
...
Agent (后台子进程)
  └── Bash echo >> ~/.claude/orchestrator/events/<orch-id>.jsonl (append-only)
       └── Coordinator 通过调度循环每周期读取新事件（基于 seq_tracker/<orch-id>.seq 游标）
            └── 读取 events/<orch-id>.jsonl 中 sequence > last_seq 的新行
                 └── 按序消费 → Compact/Detail/Summary 展示
                 └── 更新游标文件
```

事件消费策略：调度循环内联轮询（非持续 Monitor 监听）。每个调度循环周期读取增量事件，批量展示。避免 Monitor per-line 通知导致的通知风暴（一次编排可能产生 50-200 个事件）。
...
Agent 输出 → Verify Gate:
  ├── Light/Standard:
  │     → Verifier(模型: Sonnet/DeepSeek-v4-flash) 评判
  │     → 生成 Verdict: {pass: true|false, score: 0-100, issues: [...], suggestion: "..."}
  │     ├── pass=true  → 解锁下游任务，继续 DAG
  │     └── pass=false → 退回上游 Agent 修正（附带 issues + suggestion）
...
[Output Format:
  {
    "pass": true|false,
    "score": 0-100,
    "issues": [{"severity": "critical|major|minor", "location": "...", "description": "...", "suggestion": "..."}],
    "summary": "整体评价一句话"
  }
]
---
...
共享上下文操作（可选，非阻塞）：
1. 任务开始时：
   - 如果 ~/.claude/orchestrator/output/<orch-id>-shared.jsonl 已存在，读取最新 10 行了解并行 Agent 进展
2. 发现关键信息时（2-5 条）：
...
2. 移动检查点到 `~/.claude/orchestrator/checkpoints/archive/`
