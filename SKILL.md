...
```
前置步骤（调度循环外，一次性执行）:
  → 初始化 ~/.claude/orchestrator/events/<orch-id>.jsonl（空文件）
  → 初始化 ~/.claude/orchestrator/seq_tracker/<orch-id>.seq（写入 0）
...
Agent (后台子进程)
  └── Bash echo >> ~/.claude/orchestrator/events/<orch-id>.jsonl (append-only)
       └── Coordinator 调度循环每周期通过 seq_tracker 游标读取新行
            └── 按 sequence 字段有序消费 → 批量格式化展示（避免通知风暴）
```
