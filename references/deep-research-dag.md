...
## 前置阶段：研究简报与课题拆解（Coordinator 内部）

在 DAG 任务创建之前，Coordinator 先执行**轻量方案阶段**（详见 skill.md §1.5B）：
...
用户研究问题
  ...
  ▼
┌─────────────────────────┐
│ B.1 研究简报             │  Coordinator 生成 research-brief.md
│ - 核心问题（一句话）      │  （需求理解复述 + 拆解维度 + 范围边界）
│ - 拆解维度               │
│ - 范围边界               │
│ - 不确定项               │
└───────────┬─────────────┘
            ...
            ▼
┌─────────────────────────┐
│ B.2 歧义澄清（条件触发）  │  问题过于宽泛/缺少时间范围/缺少深度要求
│ - 有歧义? → 澄清后继续   │
│ - 无歧义? → 直接进入拆解  │
└───────────┬─────────────┘
            │ 简报确认
            ▼
┌─────────────────────────┐
│ 课题拆解                 │  Coordinator 将研究维度细化为搜索关键词
│ （SOP Phase 1）          │  → [HITL: 研究方向确认]
└───────────┬─────────────┘
            │
            ▼
     进入 DAG 任务拆解（并行搜索）
...
**方案输出物：** `~/.claude/orchestrator/output/<orch-id>/research-brief.md`

## DAG 结构

```
              ┌──────────────┐
              │  Coordinator  │  研究简报 + 课题拆解 → 生成搜索维度
              └──────┬───────┘
                     │
       ┌─────────────┼─────────────┐
       ▼             ▼             ▼
