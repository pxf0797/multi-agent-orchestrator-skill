...
## 前置阶段：方案设计与自审查（Coordinator 内部）

在 DAG 任务创建之前，Coordinator 先执行**方案设计与自审查**（详见 skill.md §1.5）：
...
**方案输出物：** `~/.claude/orchestrator/output/<orch-id>/initial-plan.md`
**方案摘要注入：** Architect Agent prompt 的 `[Design Context]` 段
...
[Design Context] — Coordinator 前置方案摘要:
<从 initial-plan.md 提取的方案摘要: 需求理解/技术思路/关键决策/风险点>
注意: 以上是 Coordinator 的初步方案，你应在此基础上深化和修正，而非从零开始。如发现方案存在重大问题，在输出中明确标注。
...
Coordinator 先执行 §1.5 方案设计与自审查：
1. 生成 initial-plan.md（需求理解: 4 个独立模块 + JWT 中间件、技术选型、接口草案）
2. 自审查发现: 密码重置依赖登录模块的 token 生成逻辑 → 记录到接口契约
3. 歧义检查: "密码重置"是通过邮箱链接还是短信验证码？→ 若用户未指定，触发 HITL input gate 澄清
4. 方案确认后进入拆解
